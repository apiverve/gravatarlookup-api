...
class GravatarlookupValidationException implements Exception {
  final List<String> errors;

  GravatarlookupValidationException(this.errors);

  @override
  String toString() => 'GravatarlookupValidationException: ${errors.join("; ")}';
}
...
/// Gravatar Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/gravatarlookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [email] (required) - The email address to lookup the Gravatar profile of [format: email]
class GravatarlookupClient {
  final String apiKey;
  ...
  GravatarlookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/gravatarlookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [GravatarlookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw GravatarlookupValidationException(errors);
    }
  }

  /// Execute a request to the Gravatar Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<GravatarlookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw GravatarlookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return GravatarlookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw GravatarlookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw GravatarlookupException('Resource not found');
      } else {
        throw GravatarlookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is GravatarlookupException) rethrow;
      throw GravatarlookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Gravatar Lookup API client.
class GravatarlookupException implements Exception {
  final String message;

  GravatarlookupException(this.message);

  @override
  String toString() => 'GravatarlookupException: $message';
}
